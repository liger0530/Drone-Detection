...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, cfg, dataset, use_gpu=True):
        ...
        if dataset == 'voc':
            self.threshold = cfg.VOC.OVERLAP_THRESH
            self.match = match
    ...
    def forward(self, predictions, targets):
        ...
        # Ensure tensors are on the same device as loc_data
        ...
        for idx in range(num):
            ...
            # Move defaults to same device as truths to avoid device mismatch
            if defaults.device != truths.device:
                defaults = defaults.to(truths.device)
            self.match(self.threshold, truths, defaults, self.variance, labels,
                       loc_t, conf_t, idx)

        # Removed Variable wrapper for Python 3.13.5 compatibility
